...
pub fn get_haproxy_transform_config() -> Vec<Transform> {
...
mod tests {
    use crate::transform::haproxy_transforms::{
        get_haproxy_index_missing_field, get_haproxy_transform_config, parse_to_new_haproxy_field,
    };
    use config::Config;
    use std::collections::HashMap;
    ...
    fn test_update_index() {
        ...
        let elastic_pass = settings_map
            ...
            .expect("COULD NOT GET elastic_pass")
            .as_str();
        let _run_lm_on_start = settings_map
            .get("run_lm_on_start")
            .expect("COULD NOT GET run_lm_on_start")
            .as_str();
        let _alerting_enabled = settings_map
            .get("alerting_enabled")
            .expect("COULD NOT GET alerting_enabled")
            .as_str();

        let rt = tokio::runtime::Runtime::new();
        let _out = rt.unwrap().block_on(get_haproxy_index_missing_field(
            "haproxy*",
            "new_field",
            elastic_url,
            elastic_user,
    ...
    fn test_get_missing_field() {
        ...
        let rt = tokio::runtime::Runtime::new();
        let out = rt.unwrap().block_on(get_haproxy_index_missing_field(
            "haproxy*",
            "new_field",
            elastic_url,
            elastic_user,
